...
export function SearchScreen(props: {
  ...
  return (
    <>
      ...
      <div className="card" aria-label="Text search">
        <input
          className="field"
          value={query}
        ...
        />
        <div className="pillRow" role="tablist" aria-label="Filter tabs" style={{ marginTop: 12 }}>
          {filterOptions.map((opt, idx) => (
            <button
              ...
              aria-label={`Filter, ${opt}, ${idx + 1} of ${filterOptions.length}, ${filter === opt ? "selected" : "not selected"}`}
              onClick={() => {
                setFilter(opt);
                speak(`Showing ${opt}`, { enabled: props.tts.enabled, rate: props.tts.rate, priority: "queue" });
              }}
            >
              {opt}
            </button>
      ...
      {!showResults ? (
        <section className="card" aria-label="Recent searches">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button
              ...
              onClick={() => {
                speak("Cleared recent searches", { enabled: props.tts.enabled, rate: props.tts.rate, priority: "interrupt" });
                setRecents([]);
              }}
            >
              Clear
        ...
        <section className="card" aria-label="Results list">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div style={{ fontWeight: 900 }}>Results for “{query.trim()}”</div>
